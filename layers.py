...
class Linear(nn.Module):  # B*H*W -> B*H'*W adjusted the columns in each batch, no touch to rows directly
    def __init__(self, d_in, d_out, bias = False, dropout = 0.5):
        super().__init__()
        self.M = nn.Parameter(torch.randn(d_out, d_in, requires_grad = True)*((1/(d_in+d_out))**(0.5)))  # Kaiming init
        self.bias = bias
        if self.bias:
...
class FFN(nn.Module):
    def __init__(self, d_in, expansion_size = 2, dropout = 0.2, activation = nn.ReLU(), bias = True) -> None:
        super().__init__()
...
class Upsampling(nn.Module):
    def __init__(self, 
        ...
        self.dense = Linear(d_out, d_out, bias = True, dropout = 0.2) ### is is for final layer 
...
class single_head_attention(nn.Module): 
    def __init__(self, d_in, d_out, lag = 512, dropout = 0.2, causal = True):
        ...
        if self.causal:
            self.causal_factor = nn.Parameter(torch.tril(-torch.inf*torch.ones(self.W,self.W, requires_grad = False), diagonal = -1))
